...
def generate_slurm_script(files: List[str], slurm_args: Dict[str, str]) -> str:
    ...
    print_status(f"Slurm script created and made executable", "✅")
...
def run_on_slurm(script_path: str) -> bool:
    ...
    try:
        ...
        print_status(f"Job submitted successfully: {result.stdout.strip()}", "✅")
        return True
    except subprocess.CalledProcessError as e:
        print_status(f"Error submitting job: {e}", "❌")
        print(f"stderr: {e.stderr}")
        return False
    except FileNotFoundError:
        print_status("sbatch command not found. Make sure you're on a Slurm cluster.", "❌")
        return False
